...
CrossDomainValidator::CrossDomainValidator(MusicTrainer::music::rules::ValidationPipeline& pipeline, music::ports::ScoreRepository& repository)
    : pipeline(pipeline), repository(repository) {
    // Register all rules with appropriate priorities
    pipeline.addRule(MusicTrainer::music::rules::ParallelFifthsRule::create(), {}, 10);
    pipeline.addRule(MusicTrainer::music::rules::ParallelOctavesRule::create(), {}, 10);
    pipeline.addRule(MusicTrainer::music::rules::VoiceLeadingRule::create(), {"Parallel Fifths Rule", "Parallel Octaves Rule"}, 5);
}

ValidationResult CrossDomainValidator::validate(const MusicTrainer::music::Score& score) {
    bool isValid = true;
    ...
    try {
        ...
        MusicTrainer::music::Score tempScore(scoreSnapshot);
