...
public:
    ...
    void appendEvents(const std::string& name, const std::vector<std::unique_ptr<MusicTrainer::music::events::Event>>& events);
    void createSnapshot(const std::string& name);
    std::vector<std::unique_ptr<MusicTrainer::music::events::Event>> getEvents(const std::string& name, size_t fromVersion = 0);

private:
    EventSourcedRepository() = default;
    
    struct ScoreState {
        std::vector<std::unique_ptr<MusicTrainer::music::events::Event>> events;
        std::unique_ptr<::music::events::Snapshot> snapshot;
        size_t version{0};
    ...
    void updateState(const std::string& name, const std::function<void(ScoreState&)>& updateFn) {
        ...
        if (it != scoreStates.end()) {
            ...
            if (it->second.snapshot) {
                newState.snapshot = it->second.snapshot->clone();
            }
