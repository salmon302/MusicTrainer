...
protected:
    ...
    using EventBus = music::events::EventBus;
    using EventStore = music::events::EventStore;
    using ScoreUpdatedEvent = music::events::ScoreUpdatedEvent;

    // State management
    using StateSynchronizationManager = music::statesync::StateSynchronizationManager;
...
TEST_F(StateSynchronizationManagerTest, ConflictResolution) {
    ...
    voice2->addNote(Pitch::create(Pitch::NoteName::G, 4), Duration::createQuarter().getTotalBeats());
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            voice->addNote(
                ...
                Duration::createQuarter().getTotalBeats()
            );
        }
        score->addVoice(std::move(voice));
    }
